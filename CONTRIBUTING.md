...
frontmatter 保持单行键值：`description` 不使用 `|`/`>` 块，`metadata` 是单行 JSON 对象。更长的触发说明放到正文中。
...
- `scripts/static-check.sh` — 结构化解析 frontmatter、精确 Markdown 路径/锚点、Agent 引用与 references 可达性；禁止跨 Skill 文件引用
- `python3 scripts/skill-numbering.py check` — 工作流编号连续性、引用可绑定性及小数标签守卫
- `scripts/check-current-skill-contracts.sh` — 按 `scripts/current-contract.json` 校验当前版本 / Phase / schema / 主产物 / 小节大纲契约，并拦截历史路径与静默兼容分支
- `python3 scripts/test-current-skill-contracts.py` — current-contract manifest 与主产物 fail-fast 语义回归
- `scripts/check-hook-regex-sync.sh` — hook 毒句式规则 js↔py 同步锁
...
以上为代表性列举；**强制清单按 `.github/workflows/cross-platform.yml` 为准**，每个脚本的用途与触发时机见 [scripts/README.md](scripts/README.md)。另有 `.github/workflows/cli-compat.yml` 在相关 PR、每周定时和手动触发时安装官方当前版本，真实运行 Codex 的无鉴权 smoke。
...
CLAUDE_REAL_CHECK=1 bash scripts/test-codex-cli-e2e.sh
```
...
- 执行模型：本包默认不部署 custom agents，专业分工由默认 agent 直接执行；如需多 Agent，可自行按 skill 描述部署 `.codex/agents/*.toml`。
...
- **agent 文件格式（可选部署）**：Codex custom agents 是 `.codex/agents/{name}.toml`，必需 `name`、`description`、`developer_instructions`；只读 agent 使用 `sandbox_mode = "read-only"`。
- **custom-agent 运行时注册（可选）**：自行部署 `.codex/agents/*.toml` 后，需要 trust 项目 `.codex/` 配置层并新开 Codex 会话。若当前 Codex 运行时仍返回 `unknown agent_type`，skill 必须降级默认 agent 并报告 fallback。
