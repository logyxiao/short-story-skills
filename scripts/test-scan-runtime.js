...
function testScraperImports() {
  ...
  assert(scraperPaths.length >= 2, "expected all short-rank scraper modules");
...
function runSetupCdp(scenario) {
  const setup = path.join(
    repoRoot,
    "skills/story-short-scan/scripts/setup-cdp-chrome.js"
  );
...
function testCdpProbeUsesFreshSocket() {
  const src = fs.readFileSync(
    path.join(repoRoot, "skills/story-short-scan/scripts/setup-cdp-chrome.js"),
    "utf8"
  );
