...
def read_hook_input() -> dict[str, Any]:
    ...
    # payloads carry Chinese 正文 paths, and Windows Python defaults stdin to the
...
_NET_HARD_PATTERNS = [
    ...
    (re.compile(r"(小节大纲|情节点|功能标签|目标情绪|任务描述)"), "工程词泄漏"),
    (re.compile("�"), "乱码（替换字符）"),
]
...
def toxic_phrase_findings(text: str) -> list[str]:
    ...
    for line_no, masked in content[cut:]:
        ...
        if m:
            findings.append(f"第{line_no}行 毒句式[trailer-ending]：『{m.group(0)[:20]}』——删收尾预告腔，用正在发生的动作或画面收尾。")
        ms = _TOXIC_TRAILER_SUMMARY.search(masked)
        if ms:
            findings.append(f"第{line_no}行 毒句式[trailer-summary]：『{ms.group(0)[:20]}』——删章尾状态总结句，收束状态是 小节大纲 的规划口径，正文落到具体动作、画面或台词上。")
    if findings:
...
def extract_prose_targets_from_command(command: str, depth: int = 0) -> list[str]:
    ...
    # 正文.md and wrongly deny the edit.
    targets: list[str] = []
    ...
    for raw_segment in _shell_segments(scannable):
        ...
        # 末位取到 book/正文.md —— 判到另一本书上（那本有 小节大纲 就直接放行）。
...
def extract_apply_patch_targets(command: str) -> list[str]:
    ...
    # （apply_patch 的改名/搬家形态），落盘路径是**目的地**，源路径搬完就不存在了：一份没大纲的
    # 草稿曾能靠 `Update File: draft.md` + `Move to: 书/正文.md` 直接搬进 正文（大纲门放行、
...
def staged_markdown_warnings(root: Path) -> str:
    ...
    for raw in proc.stdout.split(b"\0"):
        ...
        if file == "正文.md" or "/正文.md" in file:
            hits = []
