...
> 这一版把工具集收窄为**纯短篇网文写作**：三个长篇 skill（`story-long-scan` / `story-long-analyze` / `story-long-write`）与整套长篇逻辑（追踪模型 `tracking_commit.py`、`追踪/` 目录、卷纲/细纲、日更/回炉、黄金三章、伏笔/时间线/角色快照）整体移除。hooks、agents、部署模板、Commands、Dashboard、测试与文档全部按短篇工程（`正文.md` 单文件 + `设定.md` + `小节大纲.md` + `对标/`）重排：正文守卫只拦「首建 `正文.md` 缺 `小节大纲.md`」，部署 agent 从 7 个减到 5 个（`chapter-extractor`、`story-explorer` 随长篇移除），Skill/Command 数量从 13 降到 10，`.story-deployed` 契约升级到 `agents_version: 26`。**本版 `agents_version` 为 26**（v0.7.5 是 24），已部署的项目要重新跑 `/story-setup` 并新开会话。
...
然后在写作项目根目录重跑 `/story-setup`，并**新开一个会话**（专业 agent 只在会话启动时注册）。旧部署里的长篇 hooks（`validate-story-commit.sh`、`session-end.sh`）注册会被合并脚本按已知管理身份剥离，不残留指向已删文件的坏 hook；旧项目里残留的长篇结构（`追踪/`、`正文/第N章_*.md`、`大纲/细纲_*`）不会被删除也不会参与短篇流程，短篇写作按 `正文.md` / `设定.md` / `小节大纲.md` 识别。

### 移除

- **三个长篇 skill 整目录移除**：`story-long-scan`（起点/番茄/晋江等榜单爬虫与选题决策）、`story-long-analyze`（黄金三章/逐章摘要/剧情聚合/文风统计管道）、`story-long-write`（开书/卷纲/细纲/日更/回炉）。
- **长篇追踪模型移除**：`tracking_commit.py` 与其全部派生视图（续写状态卡、逐章记录、角色快照、伏笔表、时间线双视图）不再存在；`validate-story-commit.sh`、`session-end.sh` 两个 hook 删除；SessionEnd 与 commit 校验不再注册。
- **两个长篇专属 agent 移除**：`chapter-extractor`（逐章摘要）与 `story-explorer`（追踪/伏笔查询）；`story-review` 与 `story` 路由的查资料查询改回主线程直接检索。
- **demo 长篇示例移除**：`demo/长篇/`、`demo/拆文库/盘龙/`、`demo/长篇-*.txt` 删除，只留短篇示例。

### 变更

- **写正文守卫收窄到短篇**：`guard-outline-before-prose.sh` 与共享 JS 核只拦「首建 `正文.md` 且同目录有 `设定.md` 时缺 `小节大纲.md`」；细纲门、追踪检查点门、欠账门、字数/章名去重全部移除。`detect-story-gaps.sh` 只报「有 `正文.md` 缺 `小节大纲.md`」与拆文未完成；`session-start.sh` / `pre-compact` / `post-compact` 不再读 `追踪/上下文.md`。
- **部署物数量**：Skill/Command 13 → 10，agent 7 → 5；`story-setup` 部署清单、模型分级、安装报告、7 个 AGENTS/CLAUDE 模板全部同步。
- **story-import 只留短篇导入**：删除 Phase 3-L（长篇结构迁移）、旧追踪迁移、大型作品处理，导入产物固定为 `正文.md` / `设定.md` / `小节大纲.md` / `对标/`。
- **story-review 只留短篇审查**：删除追踪文件维护段与起点 rubric，跨批连续性改读 `设定.md` 伏笔登记。
- **Dashboard 只认短篇项目**：目录识别从「正文/大纲/设定/追踪 目录」收窄为「`正文.md` + `小节大纲.md`/`设定.md`」。
- **契约与 CI 收缩**：`current-contract.json` 去掉细纲必填项、主对标产物与 demo 细纲断言；追踪相关测试（`test-tracking-*`、`test-story-continuity`）删除，其余测试 fixture 全部改为短篇场景；`agents_version` 25 → 26，`setup_skill_version` 1.2.7 → 1.2.8。
