...
自然语言同样触发：「这篇太 AI 了」→ deslop，「打开工作台」→ `$story dashboard`（本机浏览拆文库与写作项目，可轻量编辑）。
...
`.active-book`：项目根目录的文本文件，内容为当前活跃书目的相对路径（如 `短篇/我的小说`），hook 与写作 skill 据此定位当前项目。
