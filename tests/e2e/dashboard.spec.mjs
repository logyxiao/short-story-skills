...
async function createShortProject(request, name = "短篇项目", extraFiles = {}) {
  ...
  await writeFile(resolve(projectRoot, "小节大纲.md"), "# 小节大纲\n", "utf8");
  for (const [fileName, content] of Object.entries(extraFiles)) {
    await writeFile(resolve(projectRoot, fileName), content, "utf8");
  }
  return projectRoot;
}
...
test("用现有 demo 浏览拆文库、在自建短篇项目里搜索并编辑保存", async ({ page, request }) => {
  ...
  // 清空搜索后回到已加载目录，作者刚展开的「短篇项目」不能被收起。
...
test("保存途中切走文稿，结果不能落到新打开的那份上", async ({ page, request }) => {
  ...
  await expect(page.locator("#editorTitle")).toHaveText("写作手法.md");
  ...
  await expect(page.locator("#toastRegion")).toContainText("已保存《写作手法.md》");
  await expect(page.locator("#dirtyStatus")).toContainText("待保存");
  await expect(page.locator("#saveButton")).toBeEnabled();
...
test("打开文稿不会收起正在翻的目录", async ({ page, request }) => {
  ...
  await page.locator("#treeSearch").fill("小节大纲");
  await expect(page.locator("#fileTree")).not.toContainText("正文.md");
...
test("@mobile 手机视口仍可从真实短篇项目打开大纲", async ({ page, request }) => {
  await createShortProject(request, "短篇项目");
  await page.goto("/");
  await expect(page.getByText("OH STORY", { exact: true })).toBeVisible();
