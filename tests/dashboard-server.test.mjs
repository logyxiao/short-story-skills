...
// 短篇工程根直接放 正文.md/设定.md + fileCount 个普通文件（共 fileCount + 2 项）。
async function createOversizedWorkspace(fileCount = 205) {
  ...
  await writeFile(resolve(library, "拆文报告.md"), "# 示例拆书\n", "utf8");
  for (let start = 0; start < fileCount; start += 200) {
    await Promise.all(
      Array.from({ length: Math.min(200, fileCount - start) }, (_, offset) =>
        writeFile(
          resolve(project, `普通文件_${String(start + offset + 1).padStart(5, "0")}.md`),
          "初稿",
          "utf8",
        ),
      ),
...
async function createDeepSearchWorkspace() {
  ...
  // 两个短篇工程同名文件很多（正文.md/设定.md），用 B 独有的文件名做搜索词，
  // 避免 A 的根文件也命中把结果集污染掉。
  await writeFile(resolve(deepProject, "正文.md"), "深项目正文", "utf8");
  await writeFile(resolve(deepProject, "设定.md"), "设定", "utf8");
  await writeFile(resolve(targetProject, "正文.md"), "目标正文", "utf8");
  await writeFile(resolve(targetProject, "设定.md"), "设定", "utf8");
  await writeFile(resolve(targetProject, "目标正文.md"), "目标正文", "utf8");
  return root;
}
...
describe("workspace scanning", () => {
  test("recognizes standard short projects without treating loose files or libraries as projects", async () => {
  ...
  test("discovers roots without recursively serializing every manuscript", async () => {
    ...
    assert.equal(workspace.stats.projects, 0);
  ...
  test("loads only one directory level and keeps infrastructure folders hidden", async () => {
    ...
    assert.deepEqual(
      page.entries.filter((entry) => entry.type === "file").map((entry) => entry.name).sort(),
      ["封面.png", "设定.md", "小节大纲.md", "正文.md"].sort(),
    );
  ...
  test("paginates a wide directory without dropping or duplicating files", async () => {
    ...
    // 巨书 = 正文.md + 设定.md + 205 个普通文件 = 207 项
    assert.equal(second.entries.length, 7);
    assert.equal(second.nextCursor, null);
    assert.equal(new Set([...first.entries, ...second.entries].map((entry) => entry.path)).size, 207);
  });
  ...
  test("reports result-limit and node-budget truncation independently", async () => {
    ...
    const byResults = await searchWorkspace(resultRoot, "普通", "projects");
...
describe("HTTP API", () => {
  test("serves lazy roots, directory pages, and on-demand search", async () => {
    ...
    assert.doesNotMatch(JSON.stringify(tree), /\.git|node_modules/);
