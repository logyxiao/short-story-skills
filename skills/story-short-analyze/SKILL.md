...
version: 3.0.0
description: "短篇网文拆文。拆解爆款短篇小说（番茄短篇 / 故事会 / 知乎盐选 / 追妻 / 世情 / 重生 / 虐渣等通俗题材）的故事核、结构、情感线、反转设计、写作手法、共鸣层次。单一全量拆解管道：跑完 Stage 2-6 产出完整拆文报告，落盘到 小说工作室/拆书/{书名}/，下游 story-short-write 同时读拆文报告 + 情节节点 + 写作手法 + 原文 + _meta.json 写下一篇。触发方式：/story-short-analyze、/短篇拆文、「拆短篇」「拆这篇短文」「短篇拆文」「精细拆解短篇」「8000 字短篇拆解」「番茄短篇拆文」「故事会拆解」「盐言故事拆解」「分析这篇短篇」——均进入同一管道。"
...
```
word_count = 全文字数
  └─ 直接进入 short 管道（本工具集为纯短篇拆解，不按字数路由到长篇）
```
...
进入管道前检查 `小说工作室/拆书/{书名}/_meta.json`：

```
存在 _meta.json？
  ├─ 否 → 直接进入新一轮拆解
  └─ 是 → 询问用户三选一：
       (a) 覆盖：归档旧产出到 小说工作室/拆书/{书名}/_archive_{时间戳}/ 后从 Stage 2 重跑
...
输出到 `小说工作室/拆书/{书名}/`（项目根目录下）。用户指定了其他路径时按用户指定路径输出。
...
1. 检查 `小说工作室/拆书/{书名}/原文/` 目录是否已存在
2. 如果不存在，从用户提供的源路径复制原文文件到 `小说工作室/拆书/{书名}/原文/`
3. 如果用户未提供源文件路径（直接在对话中贴文本），将原始文本保存到
   `小说工作室/拆书/{书名}/原文/原文.md`
