 ...
 * 点众阅读短篇采集脚本
 *
 * 配合 story-short-scan 采集流程使用。先启动 Chrome CDP 环境，再运行本脚本。
 ...
 *
 * 前置：
 *   node {SKILL_DIR}/story-short-scan/scripts/setup-cdp-chrome.js 9222
 */
...
function scrapeChannel(port, channelId) {
  ...
  try {
    ...
    if (!probe) {
      console.error(
        `  ✗ CDP 无响应。请确认已用 scripts/setup-cdp-chrome.js 启动 Chrome（端口 ${port}），且 agent-browser 可用。`
      );
      return null;
    }
