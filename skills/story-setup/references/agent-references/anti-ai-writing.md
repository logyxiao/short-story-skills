...
> **爆款语料校准**（盐言体短篇语料）：逗号之间平均 8.8-9.6 字；整句平均 22-24 字；逗号长句占叙述句 74-80%；≤5 字的短片段约占两成，多是孤立的时间词、转折、动作重拍。短篇段落更短（≤15 字的单句段可近一半），但句子内部的节奏不变：**段落随体裁变短，句子内部不碎**。
...
跨段「不是A。/也不是B。/只是C。」由 `formulaic-parallelism` 作 advisory：它可能是工整铺排，也可能承担辩解、悬念排除或情绪递进；只有重复提纲、拖慢画面时才压缩。该类提示与「至于X不X，怎么X」、同动词「不V A，不V B」都只作语义复核：对话也要检查，但有明确人物声线或任务功能时可保留；若来自小节大纲多个字段对同一要求的重复，正文只能消费一次，不能逐项复述。
