...
>
> **适用范围**：段落格式（戏剧单元/镜头优先，短段为底色，长段用于完整推理、氛围和情绪链）和对话格式适用于所有体裁。小节(beat)结构和字数标准仅适用于短篇。
...
**规则**：全文统一一种格式，不要混用。短篇默认纯数字 `1.`，简洁高效。
...
|--------|------|----------|
| **默认** | `「说话内容」` | 全文统一，不分平台 |
| 平台/项目指定 | `"说话内容"` | 番茄、红果、黑岩等要求半角引号时 |

**默认用 `「」`**；用户或目标平台明确要求半角引号时才改为 `""`。
...
> 付费点即免费试读和付费正文的分界，卡在卡脖子断点上。本仓库成稿固定用 `【截断点】` 标注。

- **位置**：全文 30%～50% 左右（按目标字数换算节位，在 小节大纲.md 里先定好，成稿后按实际字数复查）。不得早于 30% 或晚于 50%。
- **标注方式**：`【截断点】` 三个字**单独占一行**，前后各一个 `\n`，**不得连在正文句子后面**，也不得放在段落开头当前缀。
- **断点质量**：卡在一个即将揭晓、读者必须付费才能看到的反转或对决瞬间（亲子鉴定撕开一半、底牌刚要拍上桌、名字念到一半）。别把核心大爽点在免费段全放完。
- **正确**：`……只差最后一行。\n【截断点】\n我深吸一口气……`；**错误**：`她攥着那张纸【截断点】，手抖得拿不稳。`
...
1. **一个主事件** + **3-5 个子事件**（主事件推进核心情节，子事件丰富层次；初稿偏短时先回到小节大纲补足计划内子事件、对话或冲突，再写正文；去AI味已有正文时不得新增剧情）
2. **一个情绪变化**（读者的感受如何变化）
3. **一条读者新获知的信息**
4. **3-5 轮对话交锋**（揭示关系/升级冲突，参考 writing-craft.md 对话权力博弈。特定场景如独自发现/翻阅日记可标零，按当前 skill 的小节大纲/设计任务要求执行）
...
**通用原则**：用户未指定平台时，默认使用短篇通用格式（`1.` 纯数字分节 + `「」` 对话 + 顶格无缩进、段落间无空行 + 正文 30%～50% 处单独一行标 `【截断点】`）；用户或平台指定盐言风格时，`「」` 是默认合法格式。
