...
详细规则见 `references/short-format.md`，写作前必须加载。**主会话与 narrative-writer 子代理使用同一套正文格式**：正文只允许保存在 `正文.md`，正文相邻段落之间只允许一个换行符 `\n`（不得出现空行/`\n\n`），每段顶格写、不用缩进；对话统一用 `「」`；小节以纯数字 `1.` `2.` `3.`…标注（不用 `###`）；付费点自行卡在全文 30%～50% 处，在文中单独一行标注 `【截断点】`，不得连在正文句子后面。如果子代理输出与主会话格式不一致，按本格式规范重排后再写入文件。
...
**对标发现（先于下方反应式加载）**：`小说工作室/拆书/` 有拆过的短篇时，先按题材主动推荐一本对标，不要被动等用户开口。

1. `ls 小说工作室/拆书/` 列书目；先从当前项目目录名和 `设定.md`「基本信息」识别本篇标题，排除同名或来源指向当前 `正文.md` 的 `小说工作室/拆书/{当前书}/`。拆书目录中对应本书的分析属于续写基线，不是对标候选。排除后为空 → 跳过（无对标按题材包写，见 Phase 1 情绪→题材包表）。
2. 逐本读 `小说工作室/拆书/{书}/_meta.json` 的 `genre_detected`，与本篇题材比对，标 同题材 / 弱相关。
3. 有候选 → 用 AskUserQuestion 推荐（列候选书 +「不用，按题材包写」）。选定后记入本篇 `设定.md`「对标摘要」区作主对标，并把 `小说工作室/拆书/{书}/` 同步到当前作品的 `对标/{书}/`。
...
框架确定后，完成设计任务，然后在 `小说工作室/正文/{短篇标题}/` 下创建文件。用户明确指定其他输出目录时才覆盖该默认值。
...
3. 确定揭露方式 → 同上
4. 编写 小节大纲.md（格式见 writing-workflow.md）：短篇只做轻量蓝图，每节包含结构段/五段功能、人物/关系变化、因果/逻辑链、结尾承接/钩子，不套长篇完整章节蓝图。**标出付费点卡在哪一节末**（见 `submission-craft.md`「付费点」：卡脖子断点、付费点前每章 2-3 剧情点、付费点后每章多 1-2）；用反推法先想透付费点那一节，再倒排前后。**付费点位置必须在全文 30%～50% 左右**（按目标字数换算成节位，写进大纲）；成稿时在正文中付费点处**单独一行**标注 `【截断点】`，不连在正文句子后面。每节可选一个任务卡点，但必须服务情绪升级、证据推进、关系撕裂、反转铺垫或反击动作；没有就不强补
...
- 正文由默认 agent 直接写作；只有用户明确要求子代理、主会话上下文不足，或需要隔离试写时，才检查已部署的 narrative-writer agent（`.codex/agents/narrative-writer.toml`）并按 `agent_type` 调用，prompt 只传项目目录、输出文件、情绪目标、题材风格包、小节大纲、角色、主/副对标召回摘要、格式硬约束和写作硬约束。
