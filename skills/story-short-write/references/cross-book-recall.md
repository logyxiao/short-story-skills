...
## 触发
先按当前项目目录名、`.active-book` 与本书设定识别当前作品，排除同名或来源指向当前正文的 `小说工作室/拆书/{当前书}/` 与误建的 `对标/{当前书}/`；它们是本书导入分析，不是跨书样本。排除后，`小说工作室/拆书/` 或项目 `对标/` ≥2 本才启用。主对标书取 `设定/题材定位.md`「主对标书」字段，缺失则从排除后的候选中取字典序第一本（优先 `对标/`，否则 `小说工作室/拆书/`），并在 `gaps.main_benchmark_unspecified: true` 中提示用户补。
...
## 拆文字段 → 写作参考
读 `_meta.json.structure_counts` 时，按此表回查当前 skill 已登记的对应写作 reference。短篇优先走 genre-styles 题材包与 short-craft；未在当前 skill `参考资料` 表登记的文件不要跨 skill 加载。
