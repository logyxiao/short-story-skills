...
4. **本包默认执行 `solo`**：不部署项目 custom agents，多视角由默认 agent 在同一会话内按角色切换依次完成（架构 → 角色 → 文笔 → 一致性），每段结论前标注对应视角。若已在 Codex 项目手工部署 `.codex/agents/*.toml` 且当前运行时暴露同名 `agent_type`，可按 full/lean 走多 Agent 并行并报告 `Effective Mode: full/lean`；其余情况一律 `solo` 并报告 `Fallback: project custom agents unavailable -> solo`。
5. **运行时失败降级**：若 spawn 返回失败、`agent_type` 不可用或子 Agent 无法启动，停止继续 spawn，改用 `solo` 重新审查，并报告 `Fallback: spawn failed -> solo`。
...
Rubric: fanqie | zhihu | generic web-fiction
...
1. `{项目根}/.codex/skills/{规范路径}`（Codex 项目内安装）
2. `{项目根}/.agents/skills/{规范路径}`（Codex 扫描的项目 skill root，通常是指向 `skills/` 的 symlink）
3. 当前运行时加载本 skill 的目录，或其可访问的全局 skill 搜索路径中同名 `{skill-name}/...` 目录

> 靠前几层不存在是正常的，不是部署损坏。Codex 项目部署不复制 skill 本体，本 skill 由 Codex 从 skill root 加载，references 就在其中，通常命中第 3 或第 4 层。不要手工把 `references/` 复制进 `.codex/skills/`——手工副本不受 story-setup 管理，升级后会静默变旧。
...
| 平台 rubric | `story-review/references/rubrics/{fanqie,zhihu}.md` |
...
**.story-review/** 只保存审查状态，不属于小说事实记录；不得借此修改正文、设定或大纲。
...
2. **范围传递策略**：
   ...
   - **跨批连续性（分批必做）**：审每一批前，先读 `设定.md` 中登记的伏笔/贯穿道具当前行（已埋未回收项），并按上方契约把 state.md 的上一批未解决 findings 摘要作为「继承的开放项」注入 reviewer / consistency-checker prompt。新发现但尚未登记的开放钩子先列为维护候选，收尾时须有正文证据才能登记进 `设定.md`。
...
3. **读取相关支撑材料**：正文、`设定.md`、`小节大纲.md`、`对标/` 召回摘要；缺失时在报告中标记证据不足。
4. **识别目标平台并加载 rubric**：
   - 优先使用用户显式指定的平台。
...
6. **确定性预检（只报告，不修改）**：当审查范围包含本地正文文件路径时，运行本 skill 自带脚本：
   ...
   - 这三个预检脚本只读；`story-review` **不修改正文、设定或大纲文件**，需要自动修复正文时建议转 `/story-deslop`。分批审查的所有模式都可按上方契约写 **.story-review/state.md**，solo 除该状态外不写项目内容。
...
solo 模式下由当前默认 agent 依次按四个视角执行（本包不部署项目 custom agents）；若在手工部署 agents 的 Codex 项目中且实际模式为 full/lean，才用 Agent/Task 工具并行调用（Codex 原生子代理使用 `agent_type`，prompt 必须自包含项目路径、审查范围、文件路径、必要摘录、审查基准包摘要、Rubric Source 和统一 Findings Schema）。

**调用规则**：执行 Phase 0 后，只有实际模式仍是 full/lean 时才 spawn。不要 spawn 缺失 Agent。

**视角 1：story-architect**（架构视角）
- full/lean 均调用。
...
- 提示指令：
  ...
  Rubric Source: file | embedded fallback
  相关文件路径：{设定.md / 小节大纲.md}
  继承的开放项（分批审查必填，无则写「无」）：{从 设定.md 提取的已埋未回收钩子，连同上一批未解决 findings 摘要}
  ...
  11. 开头同质化（仅当本节是全书开篇/开头部分）：开局切口是不是同题材的默认套路（穿越即退婚、系统绑定、末世第一天、开场即打脸等），能不能原样换到任意同类书？"有钩子/非天气开场"不等于不同质。对照 references/plot-core-methods.md「噱头分类与开篇流程」判断——能整体换到同类书=同质化（撞题材模板至少 S2；套路化但有具体人物/处境微差 S3）。
...
**视角 4：consistency-checker**（一致性视角）
- full/lean 均调用。
...
- 提示指令：
  ...
  审查范围：{文件路径/章节/必要摘录}
  已知角色：{从设定文件提取角色列表}
  继承的开放项（分批审查必填，无则写「无」）：{从 设定.md 提取的已埋未回收伏笔，连同上一批未解决 findings 摘要}
  审查基准包摘要：{Phase 1 形成的 rubric / fallback 摘要，必须内联}
  Rubric Source: file | embedded fallback
  ...
  INHERITED_ITEMS: 逐条列继承的开放项 + 已检查 / 未能检查；本批新发现、不在 设定.md 的开放钩子单列，供主会话回写 设定.md。
...
3. **可选事实核查**：如果审查内容涉及需要验证的外部事实（历史年代、地理方位、职业细节等），默认 agent 直接用 Read/Grep 或联网检索验证；仅在手工部署 agents 且运行时暴露 `story-researcher` 的 Codex 项目中可额外 spawn 搜索验证。无法验证的项在报告中标记“需人工事实核查”。
...
| 时机 | 跳转到 | 命令 |
|---|---|---|
| 要修改查出的问题 | story-short-write | 返回写作 skill 修改 |
| 发现 AI 味需清理 | story-deslop | `/story-deslop` |
| 需要重新拆解对标书 | story-short-analyze | `/story-short-analyze` |
